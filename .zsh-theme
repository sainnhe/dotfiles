...
if autoload -U is-at-least && is-at-least 5.7 && [[ $COLORTERM == (24bit|truecolor) || ${terminfo[colors]} -eq 16777216 ]]; then
    FOREST_NIGHT_PURE_POWER_TRUE_COLOR=true
else
    FOREST_NIGHT_PURE_POWER_TRUE_COLOR=false
fi
...
() {
  ...
  if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    local ok="%F{#f6e3bc}${p}%f"
    local err="%F{#ff9094}${p}%f"
  else
    local ok="%F{$(_pp_s 011 223)}${p}%f"
    local err="%F{$(_pp_s 001 210)}${p}%f"
  fi
  ...
  if (( ${PURE_POWER_USE_P10K_EXTENSIONS:-1} )); then
    ...
    if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
      typeset -g POWERLEVEL9K_RULER_FOREGROUND='#536273'
    else
      typeset -g POWERLEVEL9K_RULER_FOREGROUND=$(_pp_c 5 238)
    fi
  else
  ...
  if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_DIR_WRITABLE_FORBIDDEN_VISUAL_IDENTIFIER_COLOR='#ecaf93'
  else
    typeset -g POWERLEVEL9K_DIR_WRITABLE_FORBIDDEN_VISUAL_IDENTIFIER_COLOR=$(_pp_c 003 216)
  fi
  ...
  if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_DIR_{ETC,DEFAULT}_FOREGROUND='#ecaf93'
    typeset -g POWERLEVEL9K_DIR_{HOME,HOME_SUBFOLDER}_FOREGROUND='#b9ce8e'
    typeset -g POWERLEVEL9K_DIR_NOT_WRITABLE_FOREGROUND='#ff9094'
  else
    typeset -g POWERLEVEL9K_DIR_{ETC,DEFAULT}_FOREGROUND=$(_pp_c 003 216)
    typeset -g POWERLEVEL9K_DIR_{HOME,HOME_SUBFOLDER}_FOREGROUND=$(_pp_c 010 150)
    typeset -g POWERLEVEL9K_DIR_NOT_WRITABLE_FOREGROUND=$(_pp_c 009 210)
  fi
  ...
  if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_VCS_CLEAN_FOREGROUND='#bdd0e5'
    typeset -g POWERLEVEL9K_VCS_UNTRACKED_FOREGROUND='#eccc8f'
    typeset -g POWERLEVEL9K_VCS_MODIFIED_FOREGROUND='#e5c6f0'
    typeset -g POWERLEVEL9K_VCS_LOADING_FOREGROUND='#7f8f9f'
  else
    typeset -g POWERLEVEL9K_VCS_CLEAN_FOREGROUND=$(_pp_c 007 153)
    typeset -g POWERLEVEL9K_VCS_UNTRACKED_FOREGROUND=$(_pp_c 002 108)
    typeset -g POWERLEVEL9K_VCS_MODIFIED_FOREGROUND=$(_pp_c 003 216)
    typeset -g POWERLEVEL9K_VCS_LOADING_FOREGROUND=$(_pp_c 011 223)
  fi
  ...
  if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_STATUS_ERROR_FOREGROUND='#ff9094'
  else
    typeset -g POWERLEVEL9K_STATUS_ERROR_FOREGROUND=$(_pp_c 001 210)
  fi
  ...
  if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND='#7f8f9f'
  else
    typeset -g POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND=$(_pp_c 002 65)
  fi
  ...
  if [[ "$FOREST_NIGHT_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_CONTEXT_{DEFAULT,REMOTE_SUDO,REMOTE,SUDO}_FOREGROUND='#84a78d'
    typeset -g POWERLEVEL9K_CONTEXT_ROOT_FOREGROUND='#ff9094'
  else
    typeset -g POWERLEVEL9K_CONTEXT_{DEFAULT,REMOTE_SUDO,REMOTE,SUDO}_FOREGROUND=$(_pp_c 008 246)
    typeset -g POWERLEVEL9K_CONTEXT_ROOT_FOREGROUND=$(_pp_c 009 210)
  fi
